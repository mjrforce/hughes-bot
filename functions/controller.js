...
exports.processWebhook = function(aresult){
  ...
  logToSalesforce(routeTo, source, sender, aresult.sessionId, aresult.result.resolvedQuery, webhookReply);
  return webhookReply;
}

function logToSalesforce(routeTo, source, sender, sessionId, botRequest, botResponse){
  console.log('Logging to Salesforce');
  var botchat;
  var qresult;
  ...
  org.authenticate({ username: constants.SF_USERNAME, password: constants.SF_PASSWORD})
    .then(function(oauth){
      ...
      var botmessage = nforce.createSObject('Bot_Chat_Message__c', {	Bot_Chat__c: botid, Bot_Request__c: botRequest, Bot_Response__c: botResponse, Type__c: routeTo});
      qresult = result;
    ...
    if(routeTo == 'CSR' && session.key == null){
      ...
      liveagent.post(session, qresult.records[0], botRequest);
    }
  });
